...
use Theme;
use App\Repository\StandardTourismPersonnalRepository;
...
class StandardTourismPersonnelController extends VoyagerBaseController
{
    ...
    public function __construct(
        ...
        StandardTourismPersonnalRepository $standardTourismPersonnalRepository,
    ...
    {
        $query = $this->standardTourismPersonnalRepository->listData();
        if(!empty($query)) {
            $paginatedItems = PaginateService::getPaginate($query['data'],6,$request);
        
            $data['data'] = $paginatedItems;
            $data['count'] = $query['count'];
            
            return view('standard-tourism-personnel',$data);
        }else {
            return view('standard-tourism-personnel');
        }
    ...
    public function store(Request $request)
    {
        ...
        if (!$request->has('_validate')) {
            ...
            $actions = "create";
            $module = "Standard Tourism Personnel";
    ...
    private function addMappingLang($request){
        if(!empty($request->master_id)){
            $data =  $this->mappingLangRepository->getTitleLastInsertStandardTourismPersonnels($request->title);
            $dataArr = array(
                'master_id'=> $data->id,
                'parent_id'=>$request->master_id,
                'code_lang'=>$request->_lang,
                'module'=>'standard-tourism-personnels',
