...
use App\Repository\BannerViewRepository;
use Theme;
class BannerController extends VoyagerBaseController
{
    ...
    protected $bannerViewRepository;
    ...
    public function __construct(
        ...
        $this->bannerViewRepository = $bannerViewRepository;
    ...
    public function banner()
    {
        ...
        if(!empty($checkOrganization)) {
            ...
            $allBanner = [];
            $allBanner = array_merge($banner['departmentIn'],$banner['departmentOut']);
            
            $data['banner'] = $allBanner;
    ...
    {
        $dataBanner = $this->bannerRepository->listUrlById($id);
        if(!empty($dataBanner)) {
            $title = $dataBanner->title;
            $checkType = $dataBanner->type;
            $bannerType = ($checkType == 1 ? "หน่วยงานภายใน" : ($checkType == 2 ? "หน่วยงานภายนอก":($checkType == 3 ? "หน่วยงานอื่นๆ" : "กรมการท่องเที่ยว")));
            $this->bannerViewRepository->addLogCounterById($id,$title,$bannerType);
            return redirect($dataBanner->link_url);
        }
    ...
    private function addMappingLang($request){
        if(!empty($request->master_id)){
            $data =  $this->mappingLangRepository->getTitleLastInsertBanner();
            $dataArr = array(
                'master_id'=> $data->id,
