...
class EbooksController extends VoyagerBaseController
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if($request->file_del != null) {
            $this->ebookRepository->deleteFileDownload($request->file_del,$id);
        }
        // Check permission
    ...
    private function addMappingLang($request){
        if(!empty($request->master_id)){
            $data =  $this->mappingLangRepository->getNameLastInsertEbook($request->name);
            $dataArr = array(
                'master_id'=> $data->id,
