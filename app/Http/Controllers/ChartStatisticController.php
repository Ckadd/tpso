...
class ChartStatisticController extends VoyagerBaseController
{
    ...
    {
        $dataById = $this->chartStatisticRepository->listDataById($id);
        
        $data['detailAll'] = $dataById;
        ...
        return view('chart-statistic-detail',$data);
    }
    ...
    private function addMappingLang($request){
        if(!empty($request->master_id)){
            $data =  $this->mappingLangRepository->getTitleLastInsertChartStatistics();
            $dataArr = array(
                'master_id'=> $data->id,
                'parent_id'=>$request->master_id,
                'code_lang'=>$request->_lang,
                'module'=>'chart-statistics',
