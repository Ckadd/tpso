...
use Theme;
use App\Repository\PublicGuideRepository;
use App\Repository\PublicGuideViewRepository;
...
class PublicGuideController extends VoyagerBaseController
{
    ...
    public function __construct(
        ...
        PublicGuideRepository $publicGuideRepository,
        PublicGuideViewRepository $publicGuideViewRepository,
    ...
    {
        $dataAllLogView  = $this->publicGuideViewRepository->listAllLogView();
        $alldata = $this->publicGuideRepository->getAll();
        $paginatedItems = PaginateService::getPaginate($alldata,10,$request);

        $data['alldata'] = $paginatedItems;
        return view('public-guide',compact('dataAllLogView'),$data);
    }

    public function detail($id){
        $this->publicGuideViewRepository->addlogByViewPage($id);
        $dataAllSocial= $this->publicGuideViewRepository->listAllLogById($id);
        $data = $this->publicGuideRepository->getDataById($id);
        return view('public-guide-detail',compact('data','dataAllSocial'));
    }
    ...
    {
        $type = 'facebook';
        $this->publicGuideViewRepository->addlogByShareSocialPage($id,$type);
    ...
    private function addMappingLang($request){
        if(!empty($request->master_id)){
            $data =  $this->mappingLangRepository->getTitleLastInsertPpublicGuide($request->title);
            $dataArr = array(
                'master_id'=> $data->id,
                'parent_id'=>$request->master_id,
                'code_lang'=>$request->_lang,
                'module'=>'public-guides',
