...
class ServiceListController extends VoyagerBaseController
{
    ...
    private function addMappingLang($request){
        if(!empty($request->master_id)){
            $serviceList =  $this->mappingLangRepository->getTitleLastInsertServiceLists($request->title);
            $dataArr = array(
                'master_id'=> $serviceList->id,
                'parent_id'=>$request->master_id,
                'code_lang'=>$request->_lang,
                'module'=>'service-lists',
