...
class CustomVoyagerBaseController extends VoyagerBaseController
{
     ...
     * @return array    $ids
     */
    public function getAllIdsByLangNotTH($slug) {
        $ids = array();
        $allLangLists = MappingLang::where('module', $slug)->where('code_lang', '!=', 'th')->get();
        foreach ($allLangLists as $list) {
            $ids[] = $list->master_id;
        }
        return $ids;
    }
    ...
    public function index(Request $request)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            // Allow By Ids
            $CPR = new CustomPermissionRepository($dataType->name);
            if ($CPR->getAssigedCustomPermission() && $CPR->getAssigedAllowIds()) {
                $allow_ids = $CPR->getAllowIds();
                $query->whereIn('id', $allow_ids);
            }

            // Deny By Ids
            if ($CPR->getAssigedCustomPermission() && $CPR->getAssigedDenyIds()) {
                $deny_ids = $CPR->getDenyIds();
            ...
            if ($CPR->getAssigedCustomPermission()) {
                ...
                foreach ($all_category as $column_name) {
                    ...
                    if (!array_key_exists($column_name, $relationship_belongsToMany_fields)) {
                        ...
                        if ($CPR->getAssigedCategoryAllowIds($column_name)) {
                            $allow_ids = $CPR->getCategoryAllowIds($column_name);
                            $query->whereIn($column_name, $allow_ids);
                        }
                        // Deny By Category Ids
                        if ($CPR->getAssigedCategoryDenyIds($column_name)) {
                            $deny_ids = $CPR->getCategoryDenyIds($column_name);
                            $query->whereNotIn($column_name, $deny_ids);
                        }
                    } else {
                        ...
                        // Allow By Category Ids
                        if ($CPR->getAssigedCategoryAllowIds($column_name)) {
                            $query->whereIn('id', $has_many_datas_ids);
                        }
                        // Deny By Category Ids
                        if ($CPR->getAssigedCategoryDenyIds($column_name)) {
                            $query->whereNotIn('id', $has_many_datas_ids);
                        }
                    }
                }
            }

            // Show just only thai lang
            $execude_ids = $this->getAllIdsByLangNotTH($slug);
            if (!empty($execude_ids)) {
                $query->whereNotIn('id', $execude_ids);
            }
    ...
    public function edit(Request $request, $id)
    {
        ...
        // Check permission
        // @Custom
        $this->authorize('customPermission.edit', [ CustomVoyagerBaseController::class, $dataTypeContent, $dataType]);
    ...
    public function destroy(Request $request, $id)
    {
        ...
        /**
         * Get ids for multilang
         * 
         */
        $delete_ids = MappingLangRepository::getAllRelateIdsForDelete($slug, $ids);
        $ids = $delete_ids['content_ids'];
        $mapping_ids = $delete_ids['mapping_ids'];
        
        // delete mapping lang
        if (!empty($mapping_ids)) {
            $res_del_mapping = MappingLangRepository::deleteByIds($mapping_ids);
        }
